...
const Trips = ({ trips }) => {
  ...
  const categories = ['adventure', 'budget', 'comfortable', 'luxury', 'family', 'culture', 'shopping', 'romantic', 'party', 'gastronomic', 'nature']
  ...
  return (
    ...
    @import url('https://fonts.googleapis.com/css2?family=Nunito:wght@300;400;600&display=swap');    </style>
      <h1>All Trips</h1>
      ...
      <button className='toggleBtn' onClick={toggleBtn}><i class="fas fa-filter fa-2x"></i></button>
      </div>
        ...
        {searchedTrips.map((trip) => {
          return (
            <div className="trip" key={trip._id}>
              ...
              <p className="daynumber"><i className="far fa-calendar-alt"></i>{' '}DAYS: {trip.days.length}</p>
            </div>
          );
        })}
      </div>
